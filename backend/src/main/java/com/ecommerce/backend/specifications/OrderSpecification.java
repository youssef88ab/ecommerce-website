...
public final class OrderSpecification {
    ...
    // * Method to combine all specifications (filters and search)
    public static Specification<Order> filterByUser(Long id , OrderStatus status , PaymentMethod paymentMethod, String search) {
        // * Starts with an empty specification, then combines using AND
        return (root, query, cb) -> {
            List<Predicate> predicates = new ArrayList<>();
            ...
            // * 4. Search Term (search) Filter: username LIKE %q% OR email LIKE %q%
            if (search != null && !search.trim().isEmpty()) {
                String likePattern = "%" + search.toLowerCase().trim() + "%";
